//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//     Runtime Version:2.0.50727.1433
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace DoubleDeckerBar.Adapters
{
    
    public class IReceiverEntryContractToViewAddInAdapter : DoubleDeckerBar.Views.IReceiverEntry
    {
        private Start9.Api.Contracts.IReceiverEntryContract _contract;
        private System.AddIn.Pipeline.ContractHandle _handle;
        private DoubleDeckerBar.Adapters.IMessageEventHandlerViewToContractAddInAdapter MessageReceived_Handler;
        private static System.Reflection.MethodInfo s_MessageReceivedEventAddFire;
		public event System.EventHandler<DoubleDeckerBar.Views.MessageReceivedEventArgs>MessageReceived{
			add{
				if (_MessageReceived == null)
				{
					_contract.MessageReceivedEventAdd(MessageReceived_Handler);
				}
				_MessageReceived += value;
				}
			remove{
					_MessageReceived -= value;
				if (_MessageReceived == null)
				{
					_contract.MessageReceivedEventRemove(MessageReceived_Handler);
				}
				}
		}
        static IReceiverEntryContractToViewAddInAdapter()
        {
            s_MessageReceivedEventAddFire = typeof(IReceiverEntryContractToViewAddInAdapter).GetMethod("Fire_MessageReceived", ((System.Reflection.BindingFlags)(36)));
        }
        public IReceiverEntryContractToViewAddInAdapter(Start9.Api.Contracts.IReceiverEntryContract contract)
        {
            _contract = contract;
            _handle = new System.AddIn.Pipeline.ContractHandle(contract);
            MessageReceived_Handler = new DoubleDeckerBar.Adapters.IMessageEventHandlerViewToContractAddInAdapter(this, s_MessageReceivedEventAddFire);
        }
        public string FriendlyName
        {
            get
            {
                return _contract.FriendlyName;
            }
        }
        private event System.EventHandler<DoubleDeckerBar.Views.MessageReceivedEventArgs> _MessageReceived;
        internal virtual void Fire_MessageReceived(DoubleDeckerBar.Views.MessageReceivedEventArgs args)
        {
            if ((_MessageReceived == null))
            {
            }
            else
            {
                _MessageReceived.Invoke(this, args);
            }
        }
        internal Start9.Api.Contracts.IReceiverEntryContract GetSourceContract()
        {
            return _contract;
        }
    }
}

